...
readonly class ProposalService extends AbstractEntityService implements ProposalServiceInterface
{
    ...
    public function getCsvRow(object $entity, ?string $type = null): array
    {
        ...
        return [
            ...
            $extraFields['status'] ?? '',
            $extraFields['status'] === StatusProposalEnum::SELECIONADA->value ? ($extraFields['ordem_prioridade'] ?? '') : ($extraFields['status'] === StatusProposalEnum::CLASSIFICADA->value ? ($extraFields['evaluation_ranking'] ?? '') : ''),
